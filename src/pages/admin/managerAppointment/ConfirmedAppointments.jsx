...
const ConfirmedAppointments = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const res = await axios.get("/admin/appointments/confirmed", { params });
  ...
  const handleCompleteOrder = async (appointmentId) => {
    if (!window.confirm("Xác nhận tiến hành đơn này ?")) return;
    try {
      await axios.put(`/admin/appointments/${appointmentId}/status`, { status: 'in_progress' });
      setCompletedId(appointmentId);
      fetchAppointments();
    } catch (err) {
      alert("Lỗi cập nhật trạng thái hoàn thành đơn");
    }
  };
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 p-8">
        <h1 className="text-2xl font-bold mb-6">Quản lý lịch hẹn - Đã xác nhận</h1>
